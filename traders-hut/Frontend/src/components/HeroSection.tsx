    ...
    <section className="hero-gradient relative overflow-hidden py-20 md:py-32">
      ...
      <div className="container relative mx-auto px-4 text-center">
        ...
        <p className="mx-auto mt-6 max-w-xl text-lg text-muted-foreground">
          Track NSE & BSE stocks, spot trends, and make data-driven decisions with live Indian market analytics.
        </p>
        <div className="mt-10 flex flex-col items-center justify-center gap-4 sm:flex-row">
          <Button size="lg" className="gap-2 bg-primary font-display font-bold text-primary-foreground hover:bg-primary/90" onClick={() => navigate("/chart")}>
            Open Charts <ArrowRight className="h-4 w-4" />
          </Button>
          <Button size="lg" variant="outline" className="gap-2 border-border font-display font-bold text-foreground hover:bg-secondary" onClick={() => navigate("/markets")}>
            <TrendingUp className="h-4 w-4" /> View Markets
          </Button>
        </div>
        <div className="mx-auto mt-16 grid max-w-2xl grid-cols-3 gap-6">
          {[{ label: "NSE Listed Stocks", value: "2,000+" }, { label: "Live Data", value: "Real-Time" }, { label: "Indian Indices", value: "4+" }].map((stat) => (
            <div key={stat.label}>
