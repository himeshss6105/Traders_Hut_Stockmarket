  ...
  <section className="hero-gradient relative overflow-hidden py-20 md:py-32">
    ...
    <div className="container relative mx-auto px-4 text-center">
      <div className="mx-auto mb-6 flex w-fit items-center gap-2 rounded-full border border-border bg-secondary px-4 py-1.5">
        ...
        <span className="text-sm text-muted-foreground">Indian Markets are live</span>
      </div>
      ...
      <p className="mx-auto mt-6 max-w-xl text-lg text-muted-foreground">
        Track every tick of NSE & BSE, spot trends before they move, and make data-driven decisions with institutional-grade analytics.
      </p>

      <div className="mt-10 flex flex-col items-center justify-center gap-4 sm:flex-row">
        <Button size="lg" onClick={() => document.getElementById("search")?.scrollIntoView({behavior:"smooth"})}
          className="gap-2 bg-primary font-display font-bold text-primary-foreground hover:bg-primary/90">
          Search Stocks <ArrowRight className="h-4 w-4" />
        </Button>
        <Button size="lg" variant="outline"
          onClick={() => document.getElementById("markets")?.scrollIntoView({behavior:"smooth"})}
          className="gap-2 border-border font-display font-bold text-foreground hover:bg-secondary">
      ...
      <div className="mx-auto mt-16 grid max-w-2xl grid-cols-3 gap-6">
        {[
          { label: "Indian Stocks Tracked", value: "50+" },
          { label: "NSE + BSE",             value: "Live" },
